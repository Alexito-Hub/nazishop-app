...
/// Obtiene la moneda basada en el código de país.
/// Mapea países comunes; puedes expandir el mapa según necesidades.
String getCurrencyForCountry(String? countryCode) {
  ...
  return currencyMap[countryCode] ??
      'EUR'; // Default EUR para España/Latinoamérica
}

/// Obtiene la moneda actual basada en el dispositivo.
String getCurrentCurrency() {
  final countryCode = getDeviceCountryCode();
  return getCurrencyForCountry(countryCode);
}

/// Formatea un número como moneda usando la moneda detectada.
String formatCurrency(num? value, {String? locale}) {
  if (value == null) return '';
  final currency = getCurrentCurrency();
  return NumberFormat.currency(
          locale: locale ?? 'es_ES', symbol: _getCurrencySymbol(currency))
      .format(value);
}
...
String _getCurrencySymbol(String currency) {
  ...
  return symbols[currency] ?? currency; // Retorna el código si no hay símbolo
}
